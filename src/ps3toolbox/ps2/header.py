...
import struct
from ps3toolbox.core.keys import NPD_KEK, NPD_OMAC_KEY2, NPD_OMAC_KEY3, SEGMENT_SIZE
...
def parse_ps2_header(header: bytes) -> dict[str, int | str | bytes]:
...
def verify_header(header: bytes) -> bool:
    ...
    return (
        len(header) >= 0x90 and
        header[0:4] == b'PS2\x00' and
        struct.unpack('>I', header[0x84:0x88])[0] == SEGMENT_SIZE
    )
